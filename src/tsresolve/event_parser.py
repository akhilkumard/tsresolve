...
RE_AT_TIME_2 = re.compile(r'at\s(?P<alt_hour>\d{1,2})')
...
RE_TIME_MOD = re.compile(r'morning|noon|afternoon|evening')
...
class RecurringEvent(object):
    def __init__(self, now_date=None, preferred_time_range=(0, 24)):
        if now_date is None:
    ...
    def parse_time(self, s, dt):
        if RE_TIME_MOD.search(s):
            if RE_AT_TIME_2.search(s):
                time_mod = re.findall(RE_TIME_MOD, s)
                at_time2 = re.findall(RE_AT_TIME_2, s)

                if time_mod[0] == "morning":
                    s = s.replace(at_time2[0], at_time2[0] + " a.m.")
                else:
                    s = s.replace(at_time2[0], at_time2[0] + " p.m.")
            else:
                return dt, True
        m = RE_AT_TIME.search(s)
        if m is None:
        ...
        if m:
            ...
            if hr is not None:
                if mn is not None:
                    return dt.replace(hour=hr, minute=mn, second=0, microsecond=0), True
                else:
                    # To accomodate missing minutes in time format ex. 9 p.m. instead 9:00
                    return dt.replace(hour=hr, minute=0, second=0, microsecond=0), True
    ...
    def parse_date(self, date_string):
        """
        1. seperating time and date string
        2. combining the result from parsedatetime
        :param date_string:
        :return:
        """
        # for handling next week of data ex. next week on friday
        day_found = re.findall(RE_DAY_NAMES, date_string)
        if day_found and date_string.__contains__("next week"):
            day_name = day_found[0]
            date_string = date_string.replace("next week", "")
            date_string = date_string.replace(day_name, "next" + " " + day_name)

        if date_string.__contains__("weekend"):
            date_string = date_string.replace("weekend", "saturday")

        if date_string.__contains__("from") or date_string.__contains__("after") or date_string.__contains__("before"):
            try:
                date_string = date_string.replace("from ", "")
                date_string = date_string.replace("after ", "")
                date_string = date_string.replace("before ", "")
            except: pass

        # for handling date of month ex. 5th of november
        month_found = RE_MONTH.search(date_string)

        if month_found:
            date_string = date_string.replace(month_found.group(),
                                              month_found.group("month") + " " + month_found.group("date"))

        # remove time from sending to parsedatetime
        m = RE_TIME.search(date_string)
        if m:
            date_subsstring = RE_TIME.sub("", date_string)
        else:
            date_subsstring = RE_AT_TIME.sub("", date_string)
        datestruct, result = pdt.parse(date_subsstring, self.now_date)

        if result:
            log.debug("parsed date string '%s' to %s" % (date_string, datestruct[:6]))
            return datetime.datetime(*datestruct[:6])
        return None
